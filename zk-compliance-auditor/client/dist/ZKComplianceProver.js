...
const MidnightSDKAdapter_1 = require("./MidnightSDKAdapter");
...
class ZKComplianceProver {
    constructor(config) {
        ...
        this.midnightSDK = new MidnightSDKAdapter_1.MidnightSDKAdapter({
        ...
        this.privateStateManager = new MidnightProtocolSDK_1.MidnightPrivateStateManager(env.ENCRYPTION_KEY, this.midnightSDK // Type assertion for compatibility
        );
        // Initialize data service
        this.dataService = new DataService_1.DataService(this.privateStateManager);
    }
    /**
     ...
     */
    async loadPrivateData(employeeId) {
        try {
     ...
     */
    async generateComplianceProof(privateData, publicData) {
     ...
     */
    async submitProofToNetwork(complianceProof) {
     ...
     */
    async runComplianceAudit(employeeId, auditParams) {
        const startTime = Date.now();
        try {
     ...
     */
    validateInputData(privateData, publicData) {
     ...
     */
    createProofHash(proof, publicData) {
        const proofString = JSON.stringify(proof) + JSON.stringify(publicData);
        return this.hashSensitiveData(proofString);
    }
}
exports.ZKComplianceProver = ZKComplianceProver;
//# sourceMappingURL=ZKComplianceProver.js.map