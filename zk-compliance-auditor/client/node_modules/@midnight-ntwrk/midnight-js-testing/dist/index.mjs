...
import { NetworkId, getNetworkId, networkIdToHex, getLedgerNetworkId, getZswapNetworkId, setNetworkId, stringToNetworkId, NetworkIdTypeError } from '@midnight-ntwrk/midnight-js-network-id';
import { createGzip, createGunzip } from 'node:zlib';
import { createReadStream, createWriteStream } from 'fs';
import fs from 'node:fs';
import { ContractState, LedgerState, nativeToken, Transaction as Transaction$1 } from '@midnight-ntwrk/ledger';
import { createBalancedTx, SucceedEntirely } from '@midnight-ntwrk/midnight-js-types';
...
const waitForFullSync = (wallet, throttleTime = 3_000) => {
    ...
    return Rx.firstValueFrom(wallet.state().pipe(Rx.throttleTime(throttleTime), Rx.tap((state) => {
...
const waitForSyncProgressDefined = async (wallet, throttleTime = 3_000) => {
    ...
    return Rx.firstValueFrom(wallet.state().pipe(Rx.timeout(30_000), Rx.throttleTime(throttleTime), Rx.tap((state) => logger.info(logState(state))), Rx.filter((state) => {
...
const syncWallet = (wallet, throttleTime = 3_000) => {
    ...
    return Rx.firstValueFrom(wallet.state().pipe(Rx.throttleTime(throttleTime), Rx.tap((state) => logger.info(logState(state))), Rx.filter((state) => state.syncProgress !== undefined && state.syncProgress.lag.applyGap < 100n), Rx.map((s) => s.balances[nativeToken()] ?? 0n), Rx.filter((balance) => balance > 0n)));
};
/**
...
class WalletFactory {
    ...
    static build = async (env, walletLogLevel = DEFAULT_WALLET_LOG_LEVEL) => {
        ...
        return WalletBuilder.build(indexer, indexerWS, proofServer, node, Buffer.from(generateRandomSeed()).toString('hex'), getZswapNetworkId(), walletLogLevel);
    };
    /**
    ...
    static buildFromSeed = async (env, seed, walletLogLevel = DEFAULT_WALLET_LOG_LEVEL) => {
        ...
        const { indexer, indexerWS, node, proofServer } = env;
        return WalletBuilder.buildFromSeed(indexer, indexerWS, proofServer, node, seed, getZswapNetworkId(), walletLogLevel);
    };
    /**
    ...
    static restore = async (env, serialized, seed, trimTxHistory = true, walletLogLevel = DEFAULT_WALLET_LOG_LEVEL) => {
        ...
        const { indexer, indexerWS, node, proofServer } = env;
        return WalletBuilder.restore(indexer, indexerWS, proofServer, node, seed, serialized, walletLogLevel, trimTxHistory);
    };
    /**
...
class MidnightWalletProvider {
    ...
    balanceTx(tx, newCoins) {
        return this.wallet
            .balanceTransaction(Transaction.deserialize(tx.serialize(getLedgerNetworkId()), getZswapNetworkId()), newCoins)
            .then((utx) => this.wallet.proveTransaction(utx))
            .then((zswapTx) => Transaction$1.deserialize(zswapTx.serialize(getZswapNetworkId()), getLedgerNetworkId()))
     ...
     */
    submitTx(tx) {
        return this.wallet.submitTransaction(Transaction.deserialize(tx.serialize(getLedgerNetworkId()), getZswapNetworkId()));
    }
    /**
    ...
    static async build(logger, env, seed, walletLogLevel = DEFAULT_WALLET_LOG_LEVEL) {
        const wallet = await WalletFactory.buildFromEnvContext(env, seed ?? Buffer.from(generateRandomSeed()).toString('hex'), walletLogLevel);
        const initialState = await getInitialState(wallet);
...
const initializeMidnightProviders = (midnightWalletProvider, environmentConfiguration, contractConfiguration) => {
    return {
        ...
        zkConfigProvider: new NodeZkConfigProvider(contractConfiguration.zkConfigPath),
...
export { DEFAULT_WALLET_LOG_LEVEL, DEFAULT_WALLET_STATE_DIRECTORY, DevnetTestEnvironment, DynamicProofServerContainer, EnvVarRemoteTestEnvironment, FaucetClient, GzipFile, IndexerClient, LocalTestConfiguration, LocalTestEnvironment, MINUTE, MidnightWalletProvider, NodeClient, ProofServerClient, QanetTestEnvironment, RemoteTestEnvironment, StaticProofServerContainer, TestEnvironment, Testnet2TestEnvironment, TestnetTestEnvironment, WalletFactory, WalletSaveStateProvider, createDefaultTestLogger, createLogger, createOutputs, defaultContainersConfiguration, delay, deleteDirectory, expectFoundAndDeployedStatesEqual, expectFoundAndDeployedTxDataEqual, expectFoundAndDeployedTxPrivateDataEqual, expectFoundAndDeployedTxPublicDataEqual, expectStatesEqual, expectSuccessfulCallTx, expectSuccessfulDeployTx, expectSuccessfulTxData, extractHostnameAndPort, getContainersConfiguration, getInitialState, getTestEnvironment, getWalletStateFilename, inMemoryPrivateStateProvider, initializeMidnightProviders, latestContainersConfiguration, logger, normalizeWalletState, processTransaction, sendTransactionToAddress, sendTransactionToWallet, setContainersConfiguration, stateValueEqual, syncWallet, txsEqual, validateFinalState, waitForFinalizedBalance, waitForFullSync, waitForFunds, waitForPending, waitForSyncProgressDefined, waitForTxInHistory, walletStateWithoutHistoryAndCoins };
